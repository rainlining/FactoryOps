...
const queueStatusNames={QUEUED:"待检测",STARTING:"正在启动",RUNNING:"检测中",QA_ACCEPTED:"质检通过",RECHECK_REQUIRED:"待复检",WAITING_FOR_APPROVAL:"待审批",APPROVED_ACTION_PENDING:"已批准·待业务执行",APPROVAL_REJECTED:"已驳回",RECHECK_REQUESTED:"已要求复检",ESCALATED:"已升级主管",FAILED:"失败",CANCELLED:"已取消"};
...
async function queueAction(itemId,action){const response=await fetch(`/api/batch-queue-items/${itemId}/${action}`,{method:"POST"});const data=await response.json().catch(()=>({}));if(!response.ok){notice(data.error||"当前批次无法执行该操作");return}notice(action==="retry"?"已创建重试批次":"已发送取消请求");await refreshQueue()}

const approvalStatusNames={PENDING:"待审批",APPROVED_ACTION_PENDING:"已批准·待业务系统执行",APPROVAL_REJECTED:"已驳回",RECHECK_REQUESTED:"已要求复检",ESCALATED:"已升级主管"};
async function refreshApprovals(){const response=await fetch("/api/batch-approvals",{cache:"no-store"});if(!response.ok)throw new Error("无法读取待审批事项");const data=await response.json();approvalRecords=data.approvals||[];const pending=approvalRecords.filter(item=>item.status==="PENDING").length;$("approvalCount").textContent=`${pending} 项待处理`;$("approvalList").replaceChildren(...(approvalRecords.length?approvalRecords.map(item=>{const card=document.createElement("button");card.type="button";card.className=`approval-list-item ${item.status==="PENDING"?"pending":"resolved"}`;card.innerHTML=`<span>${clean(item.evidence.batch_id)}</span><strong>${clean(item.recommended_action)}</strong><small>${clean(approvalStatusNames[item.status]||item.status)} · ${clean(item.evidence.risk_level)}</small>`;card.onclick=()=>showApproval(item.item_id);return card}):[Object.assign(document.createElement("p"),{className:"empty",textContent:"当前没有待审批批次。"})]));}
async function showApproval(itemId){const response=await fetch(`/api/batch-approvals/${itemId}`,{cache:"no-store"});const item=await response.json();if(!response.ok){notice(item.error||"无法读取审批详情");return}const pending=item.status==="PENDING";$("approvalDetail").innerHTML=`<div class="approval-detail-head"><div><span class="eyebrow">${clean(item.approval_id)}</span><h3>${clean(item.evidence.batch_id)}</h3></div><span class="status-pill">${clean(approvalStatusNames[item.status]||item.status)}</span></div><div class="approval-evidence"><div><span>Risk 建议</span><strong>${clean(item.recommended_action)}</strong></div><div><span>风险等级</span><strong>${clean(item.evidence.risk_level)}</strong></div><div><span>产品数量</span><strong>${clean(item.evidence.item_count)}</strong></div><div><span>策略依据</span><strong>${clean((item.evidence.policy_refs||[]).join("、")||"未提供")}</strong></div></div><section class="approval-copy"><strong>批次结论</strong><p>${clean(item.evidence.coordinator)}</p><strong>Risk / Policy 输出</strong><p>${clean(item.evidence.risk)}</p></section>${pending?`<div class="approval-form"><label>审批人<input id="approvalActor" maxlength="80" placeholder="例如：质量主管"></label><label>审批意见<textarea id="approvalComment" maxlength="500" placeholder="说明判断依据"></textarea></label><div class="approval-actions"><button class="primary-button" data-decision="APPROVE">批准建议动作</button><button class="secondary-button" data-decision="REJECT">驳回建议动作</button><button class="secondary-button" data-decision="RECHECK">要求重新检测</button><button class="secondary-button" data-decision="ESCALATE">升级给主管</button></div><p class="approval-warning">批准 HOLD_BATCH / STOP_LINE 后仅记录为“待业务系统执行”，不会伪装成产线已经动作。</p></div>`:`<div class="approval-audit"><strong>审批记录</strong><p>${clean(item.actor||"未记录")} · ${clean(item.comment||"无意见")}</p><p>${item.status==="APPROVED_ACTION_PENDING"?"审批已记录，待业务系统执行。":"该审批事项已经完成。"}</p></div>`}`;if(pending){$("approvalDetail").querySelectorAll("[data-decision]").forEach(button=>button.onclick=()=>submitApproval(itemId,button.dataset.decision));}}
async function submitApproval(itemId,decision){const actor=$("approvalActor").value.trim(),comment=$("approvalComment").value.trim();if(!actor){notice("请填写审批人");return}if(["APPROVE","REJECT"].includes(decision)&&!comment){notice("请填写审批意见");return}const labels={APPROVE:"批准建议动作",REJECT:"驳回建议动作",RECHECK:"要求重新检测",ESCALATE:"升级给主管"};if(!window.confirm(`确认${labels[decision]}？`))return;const response=await fetch(`/api/batch-approvals/${itemId}/decision`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({command_id:`CMD-${itemId}-${Date.now()}`,decision,actor,comment})});const data=await response.json();if(!response.ok){notice(data.error||"审批提交失败");return}notice(decision==="APPROVE"?"审批已记录，待业务系统执行":`${labels[decision]}已记录`);await Promise.all([refreshQueue(),refreshApprovals()]);await showApproval(itemId)}
...
$("startQueue").addEventListener("click",async()=>{const response=await fetch("/api/batch-queues/current/start",{method:"POST"});const data=await response.json().catch(()=>({}));if(!response.ok){notice(data.error||"没有可启动的批次");return}notice("连续检测已启动");if(!queuePolling)queuePolling=setInterval(()=>refreshQueue().catch(error=>notice(error.message)),900);await refreshQueue()});
...
refreshApprovals().catch(error => notice(error.message));
