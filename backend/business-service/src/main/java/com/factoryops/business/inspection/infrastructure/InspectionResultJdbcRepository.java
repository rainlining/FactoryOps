...
public class InspectionResultJdbcRepository {
  ...
  public void insert(ValidatedVisionResult result) {
    jdbc.update(
        ...
        result.resultId(),
        hash(result.inspectionId()),
        result.inspectionId(),
        result.originKind(),
        normalize(result.anomalyScore()),
        normalize(result.decisionThreshold()),
        new String(result.canonicalPayload(), StandardCharsets.UTF_8),
        result.payloadHash());
  }
  ...
  public Optional<StoredInspectionResult> findByResultId(String resultId) {
    var rows =
        jdbc.query(
            ...
            """,
            (rs, row) -> new StoredInspectionResult(rs.getString(1), rs.getBytes(2)),
            hash(resultId));
  ...
  public Optional<ResultEvidence> findEvidence(String resultId) {
    return jdbc
        .query(
            "SELECT"
                + " result_id,inspection_id,JSON_EXTRACT(canonical_payload,'$.observation.is_anomaly')"
                + " FROM vision_inspection_results WHERE result_id_hash=?",
            (rs, n) -> new ResultEvidence(rs.getString(1), rs.getString(2), rs.getBoolean(3)),
            hash(resultId))
        .stream()
