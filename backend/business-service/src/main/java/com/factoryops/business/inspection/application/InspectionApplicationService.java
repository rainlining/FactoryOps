...
public class InspectionApplicationService {
    ...
    public InspectionApplicationService(InspectionJdbcRepository r, InspectionResultJdbcRepository rr,BatchJdbcRepository b, Clock c, @Qualifier("inspectionWriteTransaction") TransactionTemplate w, @Qualifier("inspectionReadTransaction") TransactionTemplate rd){
        repository=r;resultRepository=rr;batches=b;clock=c;write=w;read=rd;
    }
    public Creation create(String id,String batchId, InspectionInput input) {
        if(batchId==null)throw new BatchIdRequiredException();
        var found = read.execute(s -> repository.find(id).map(inspection -> compare(inspection,batchId, input)));
        if(found != null && found.isPresent()) return found.get();
        var candidate=Inspection.pending(id,batchId,input,clock.instant());
        try {
            write.executeWithoutResult(s->{var batch=batches.findForUpdate(batchId).orElseThrow(BatchReferenceNotFoundException::new);if(batch.kind()!=BatchKind.PRODUCTION||batch.status()==BatchStatus.RELEASED)throw new BatchNotAcceptingInspectionsException();repository.insert(candidate);});
            return new Creation(candidate,false,0);
        }
        catch(DuplicateKeyException e){
            return read.execute(s -> repository.find(id)
                    .map(inspection -> compare(inspection,batchId, input)))
                    .orElseThrow(() -> e);
        }
    }
    ...
    private Creation compare(Inspection i,String batchId,InspectionInput input){
        if(i.batchId().equals(batchId)&&i.input().equals(input))return new Creation(i,true,resultRepository.countByInspectionId(i.id()));
