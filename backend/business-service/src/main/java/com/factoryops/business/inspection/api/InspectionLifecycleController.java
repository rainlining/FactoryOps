...
public class InspectionLifecycleController {
  ...
  ResponseEntity<InspectionLifecycleResponse> create(@RequestBody InspectionCreateRequest r) {
    ...
    var c =
        service.create(
            r.inspectionId(),
            r.batchId(),
            new InspectionInput(r.input().imageUri(), r.input().sha256()));
  ...
  private InspectionLifecycleResponse response(Inspection i, long resultCount, boolean replayed) {
    return new InspectionLifecycleResponse(
        i.id(),
        i.batchId(),
        i.status().name(),
        new InspectionLifecycleResponse.Input(i.input().imageUri(), i.input().sha256()),
        i.createdAt(),
        i.completedAt(),
        resultCount,
        replayed);
