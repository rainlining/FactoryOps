...
public class InspectionLifecycleController {
 ...
 @PostMapping ResponseEntity<InspectionLifecycleResponse> create(@RequestBody InspectionCreateRequest r){if(r.input()==null)throw new IllegalArgumentException("input is required");var c=service.create(r.inspectionId(),new InspectionInput(r.input().imageUri(),r.input().sha256()));return ResponseEntity.status(c.replayed()?HttpStatus.OK:HttpStatus.CREATED).body(response(c.inspection(),c.replayed()));}
