...
public final class VisionInspectionContractValidator {
  ...
  public VisionInspectionContractValidator(JsonMapper mapper) {
    try (var input =
        getClass().getResourceAsStream("/contracts/vision_inspection/v1.0/schema.json")) {
  ...
  public ValidatedVisionResult validate(JsonNode payload) {
    ...
    if (!"1.0".equals(version)) {
      throw issue(
          ...
          "$.contract_version",
          "Only contract_version 1.0 is supported");
    }
    ...
    if (!errors.isEmpty()) {
      var first =
          errors.stream()
              ...
              .orElseThrow();
      throw issue(
          first.getKeyword(), jsonPath(first.getInstanceLocation().toString()), first.getMessage());
    }
    var observation = payload.get("observation");
    var anomaly = observation.get("is_anomaly").booleanValue();
    var score = observation.get("anomaly_score").decimalValue();
    var threshold = observation.get("decision_threshold").decimalValue();
    try {
      InspectionResult.decision(anomaly, score, threshold);
    ...
    return new ValidatedVisionResult(
        payload.get("inspection_id").asText(),
        payload.get("result_id").asText(),
        payload.get("origin").get("kind").asText(),
        payload.get("input").get("image_uri").asText(),
        payload.get("input").get("sha256").asText(),
        anomaly,
        score,
        threshold,
        canonical,
