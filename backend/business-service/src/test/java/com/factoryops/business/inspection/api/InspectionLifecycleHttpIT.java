...
class InspectionLifecycleHttpIT {
  ...
  @Autowired InspectionApplicationService service;
  ...
  @Test
  void creates_replays_and_queries_pending_inspection() throws Exception {
  ...
  @Test
  void rejects_identity_conflict_and_reports_missing() throws Exception {
    mvc.perform(
            post("/api/v1/inspections")
                .contentType("application/json")
                .content(request("inspection-1", "artifact://images/a", "a".repeat(64))))
        .andExpect(status().isCreated());
    mvc.perform(
            post("/api/v1/inspections")
                .contentType("application/json")
                .content(request("inspection-1", "artifact://images/b", "b".repeat(64))))
        .andExpect(status().isConflict())
        .andExpect(jsonPath("$.code").value("inspection_identity_conflict"));
    mvc.perform(get("/api/v1/inspections/missing"))
  ...
  @Test
  void rejects_invalid_or_unknown_input() throws Exception {
    mvc.perform(
            post("/api/v1/inspections")
                .contentType("application/json")
                .content(request("inspection-1", "not-a-uri", "ABC")))
        .andExpect(status().isUnprocessableEntity())
        .andExpect(jsonPath("$.code").value("invalid_inspection_input"));
    mvc.perform(
            post("/api/v1/inspections")
                .contentType("application/json")
                .content(
                    "{\"inspection_id\":\"i\",\"input\":{\"image_uri\":\"artifact://images/a\",\"sha256\":\""
                        + "a".repeat(64)
                        + "\",\"extra\":1}}"))
  ...
  @Test
  void concurrent_identical_creates_choose_one_row_and_replay_the_other() throws Exception {
    ...
    try {
      var futures =
          List.of(
              executor.submit(
                  () -> {
                    start.await();
                    return service.create("inspection-1", "B-TEST", input);
                  }),
              executor.submit(
                  () -> {
                    start.await();
      ...
      start.countDown();
      assertThat(futures)
          .extracting(future -> future.get().replayed())
  ...
  private String request(String id, String uri, String sha) {
    return "{\"inspection_id\":\""
        + id
        + "\",\"batch_id\":\"B-TEST\",\"input\":{\"image_uri\":\""
        + uri
        + "\",\"sha256\":\""
        + sha
        + "\"}}";
