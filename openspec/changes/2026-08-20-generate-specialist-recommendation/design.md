...
provider 必须声明六项版本 provenance，并与 Execution 冻结值完全一致；校验在 replay shortcut 之前执行。若该 execution 的 recommendation key 已存在且 generated_at 相同，服务在调用 provider 前直接返回 existing identical，避免 replay 重复产生模型成本；这条历史 replay 不要求 parent 仍 RUNNING，但仍要求 Execution Contract 和 provenance 可信。不同 generated_at 分类 conflicting。首次生成才要求 current RUNNING pair，并构造 `SpecialistGenerationContext`：只包含 run/task/execution identity、role、task type、context snapshot ID、输入 evidence refs 与已校验 provenance，不包含 Evaluation ground truth。provider 返回 `SpecialistRecommendationDraft`，无权设置 identity、contract version 或 generated_at；draft evidence 必须是 context evidence 子集，当前 recorded 边界不接受 output artifact refs。

应用层由 execution ID 计算 recommendation key，并由 key 摘要确定性派生 recommendation ID，组合 v1.0 Contract 后调用 `SpecialistRecommendationService.save`。Provider 调用期间不持有数据库连接或事务；保存阶段 persistence 重新锁定 Task/Execution，并校验 current RUNNING ownership、二者 context snapshot 一致以及六项 Execution provenance 未变化。因此调用期间 parent 收口、snapshot 或 provenance 漂移都会导致拒绝且不留 Recommendation。
...
测试覆盖 recorded role draft、deterministic identity、RUNNING/terminal replay、provider provenance/evidence/artifact 越界、provider exception、非法 draft、上下文错配、barrier 强制的真实并发 identical，以及 provider 调用期间 parent 收口、snapshot 或 provenance 漂移。
