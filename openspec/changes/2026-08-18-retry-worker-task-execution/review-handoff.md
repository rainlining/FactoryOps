...
验证：retry `7 passed`，相关组合 `45 passed`，Agent 全量 `143 passed`，Contract `99 passed`，Java `65 tests`，Ruff/diff/dataset 检查通过。非目标为自动循环/backoff、跨 worker 接管、heartbeat、LLM/Tool 调用或业务副作用重试、Checkpoint/Resume、Java API、Evaluation、`dataset/`。
