...
公开 `validate_risk_decision` 必须接收源 Recommendation identity，并比对 recommendation ID/key、Run 与 Task；canonical/relation 仅调用内部 payload 校验，因为它们比较已经接收的 Risk 事实，不承担跨对象 admission。Canonical 递归把等值整数浮点归一化为整数，避免 `1` 与 `1.0` 形成伪冲突。
