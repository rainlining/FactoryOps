...
业务规则采用项目总纲已冻结的动作风险等级：PASS/RECHECK 为 LOW，REJECT_ITEM/HOLD_BATCH 为 MEDIUM，STOP_LINE/ESCALATE 为 HIGH。HIGH 必须等待人工审批；MEDIUM 在 Specialist 明确冲突时等待审批，否则允许；LOW 允许。当前动作集合没有确定性 BLOCK 条件，禁止为了填满枚举而虚构阻断规则。
