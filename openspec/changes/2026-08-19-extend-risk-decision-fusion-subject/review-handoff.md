...
首审发现并修复 2 个 Important：v1.1 canonical/relation 版本支持和 subject 字段互斥；同时收紧 source 参数混用。修复后 Contract 局部 10 passed、全 Contract 135 passed、Ruff/Schema 通过。Agent/Java 的 Testcontainers 全量因 Docker named pipe 不可用未通过，详见 verification.md；不得把环境失败伪装成代码通过。等待同一子 Agent 复审。
