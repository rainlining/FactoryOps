...
revision 1 必须为 PENDING；只能进入 APPROVED、REJECTED 或 EXPIRED。批准/拒绝必须是 HUMAN，过期必须是 SYSTEM；终态不得迁移。请求时间必须早于过期时间，决定时间不得早于请求时间。
...
canonical 必须稳定；相同快照为 identical，合法 revision+1 为 next revision，其余同 key 内容为 conflicting，不同 key 为 distinct。
