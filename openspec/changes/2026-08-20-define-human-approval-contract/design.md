...
状态快照包含 revision 与 status。revision 1 必须是 PENDING 且不含 outcome；下一 revision 只允许 PENDING→APPROVED/REJECTED/EXPIRED。APPROVED/REJECTED 必须由 HUMAN actor 决定；EXPIRED 必须由 SYSTEM actor 决定。终态不可继续迁移。审批批准只证明人工 Gate 通过，不表示业务动作已经执行。
