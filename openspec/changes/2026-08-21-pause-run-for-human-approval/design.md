...
- 相同 Approval 重放要求 Approval canonical identical、Run 已是 WAITING_FOR_APPROVAL，且确定性 transition 与 approval/run/revision/reason 完整一致。
...
真实 MySQL 覆盖 applied、identical replay、conflicting replay、Approval insert/transition rollback、错误 Run 状态、transition corruption，以及并发相同创建只产生一条 Approval 和一条 wait transition；随后执行 Agent 全量、Contract、Java、Ruff 与 diff/dataset 检查。
