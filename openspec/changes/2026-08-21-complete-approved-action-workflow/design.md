...
入口 `ApprovedWorkflowCompletionService.complete(terminal_approval)` 先复用 `ApprovedActionResumeService.resume`，因此 Java receipt 是业务副作用幂等锚点，Run resume transition 是工作流恢复锚点。随后在新的 Agent MySQL 事务中按既有顺序锁定 Fusion 完整 provenance、Risk Decision、Run、Approval current/history、Coordinator Execution 与两个确定性 completion transition identity。
