...
- Review/Learning 会话已完成最终接受，归档状态为 `completed`。

## 归档时已知接受事项

- 首次 dispatch 在同一事务内允许 Coordinator Execution `PENDING → RUNNING`；原始 spec/design 文案未完全同步，列为后续文档改进。
- 原始测试未真实覆盖 conflicting replay 和完整 history rollback；列为后续测试改进。
- 上述两项由项目所有者明确接受，不在本次归档中声明为已修复。
