...
项目所有者已完成 Review 并接受当前实现，同时明确保留以下事项作为后续改进，不作为本 Change 已修复内容：

- 首次 dispatch 会在同一事务内将 Coordinator Execution 从 `PENDING` 推进到 `RUNNING`，原始 spec/design 文案未完整反映这一实现语义。
- 原始测试未真实覆盖 conflicting replay 和完整 history rollback 场景。
