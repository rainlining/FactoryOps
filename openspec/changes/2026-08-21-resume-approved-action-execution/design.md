...
1. 用既有 `HumanApprovalService.save` 保存 revision 2 APPROVED fact；相同 terminal replay 必须 identical。
...
5. 在 Agent 事务内先 `FOR UPDATE` 锁 Run，再锁确定性 wait/resume transition；持有 Run fence 期间调用有限 timeout 的 Java API，随后以从 Approval ID 派生的 transition ID/request ID原子执行 `WAITING_FOR_APPROVAL → RUNNING`。
...
生产 adapter 使用 Python 标准库同步 HTTP，配置 base URL、service token 和有限 timeout；非 2xx、malformed JSON、网络/timeout 都转为稳定领域异常，不自动循环重试。service 层只依赖 Protocol，真实 MySQL saga 测试使用 recording fake；另用本地 HTTP server 做 adapter contract test。

## 测试

真实 MySQL 覆盖 applied、Java replay、terminal duplicate、Java failure、receipt mismatch、Java 成功后 transition failure再恢复、并发相同 resume、wait/history corruption。HTTP adapter 覆盖 header/path/body、2xx decode、非 2xx/malformed/timeout 分类。然后执行 Agent/Contract/Java 全量、Ruff、diff/dataset 检查。
