# Agent Run Lifecycle Persistence 规格
...
#### Scenario: 并发赢家在校验期间提交

- **GIVEN** 当前请求首次查询幂等键未命中
- **AND** 并发请求随后提交相同幂等键
- **WHEN** 当前请求在 INSERT 前遇到 Contract 或 lineage 失败
- **THEN** 当前请求必须重新查询已提交赢家并按 identical/conflicting 分类
...
#### Scenario: 迁移赢家在校验期间提交

- **GIVEN** 当前命令首次查询请求 ID 未命中
- **AND** 并发命令随后提交相同请求 ID
- **WHEN** 当前命令在 SQL 前遇到 Run、状态图或 Contract 失败
- **THEN** 当前命令必须重新查询赢家并按 identical/conflicting 分类
...
- **WHEN** 候选终态的 `ended_at` 早于已有 `started_at`
- **THEN** Application Service 必须在 SQL 写入前通过 Contract 校验拒绝
- **AND** snapshot revision 和 history 均不得改变
